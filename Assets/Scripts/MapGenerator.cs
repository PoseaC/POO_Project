    ...
    void GenerateFoliage(Vector3[] heightMap)
    {
        ...
        for(int i = 0; i < heightMap.Length; i++)
        {
            if(heightMap[i].y > treeGrowthRange.x && heightMap[i].y < treeGrowthRange.y)
            {
                ...
                {
                    PlantBehaviour tree = trees[Random.Range(0, trees.Length)];

                    Vector3 treePosition = heightMap[i];
                    Vector2 spawnDeviation = Random.insideUnitCircle * deviationFromSpawn;

                    treePosition.x = treePosition.x + spawnDeviation.x;
                    treePosition.z = treePosition.z + spawnDeviation.y;

                    Physics.Raycast(treePosition + Vector3.up * deviationFromSpawn, Vector3.down, out RaycastHit hit, deviationFromSpawn * 2);

                    Instantiate(tree, hit.point, Quaternion.Euler(0, Random.Range(0, 360), 0));
                }
                //GenerateGrass(heightMap[i]);
            }
        }
    }
    void GenerateGrass(Vector3 position)
    {
        for (int i = 0; i <= grassDensity; i++)
        {
            ...
            grassPosition.x = grassPosition.x + spawnDeviation.x;
            grassPosition.z = grassPosition.z + spawnDeviation.y;
